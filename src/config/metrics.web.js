...
const widthN = num =>
  PixelRatio.roundToNearestPixel((DEVICE_WIDTH / DEVICE_WIDTH) * num);
const heightN = num =>
  PixelRatio.roundToNearestPixel((DEVICE_HEIGHT / DEVICE_HEIGHT) * num);
...
const width = num => widthN(num);
const height = num => heightN(num);

const AVATAR_SLIDER_ITEM_WIDTH = width(225);

const scaledFontSize = fontSize =>
  Math.round((fontSize * DEVICE_WIDTH) / DEVICE_WIDTH);

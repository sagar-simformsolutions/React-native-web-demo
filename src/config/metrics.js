...
const IS_IPHONEX =
  ...
  DeviceInfo.getModel().includes('iPhone 11');
...
const DEVICE_HEIGHT =
  windowSize.height > windowSize.width
    ...
    : windowSize.width;
const DEVICE_WIDTH =
  windowSize.height > windowSize.width ? windowSize.width : windowSize.height;
...
const widthN = num =>
  PixelRatio.roundToNearestPixel((DEVICE_WIDTH / DESIGN_WIDTH) * num);
const heightN = num =>
  PixelRatio.roundToNearestPixel((DEVICE_HEIGHT / DESIGN_HEIGHT) * num);

const widthX = num =>
  PixelRatio.roundToNearestPixel((DEVICE_WIDTH / DESIGN_WIDTH_X) * num);
const heightX = num =>
  PixelRatio.roundToNearestPixel((DEVICE_HEIGHT / DESIGN_HEIGHT_X) * num);

const width = num => (IS_IPHONEX ? widthX(num) : widthN(num));
const height = num => (IS_IPHONEX ? heightX(num) : heightN(num));

const AVATAR_SLIDER_ITEM_WIDTH = IS_TABLET ? width(400) : width(225);

const scaledFontSize = fontSize =>
  Math.round((fontSize * DEVICE_WIDTH) / DESIGN_WIDTH);
...
function moderateVerticalScale(
  size: number,
  skipAspectRatio: boolean = false,
  factor: number = 0.5,
): number {
